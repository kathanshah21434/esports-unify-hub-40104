    ...
    try {
      ...
      const [tournaments, players, matches] = await Promise.all([
        tournamentService.getAll().catch(err => {
          ...
          return mockTournaments; // Fallback to mock data if database fails
        }),
        playerService.getAll().catch(err => {
          console.error('Failed to load players:', err);
          return mockPlayers; // Fallback to mock data if database fails
        }),
        matchService.getAll().catch(err => {
          console.error('Failed to load matches:', err);
          return mockMatches; // Fallback to mock data if database fails
        })
      ]);
